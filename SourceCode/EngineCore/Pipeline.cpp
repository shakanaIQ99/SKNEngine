...
PipelineSet Pipeline::CreateFBXPipeline(ID3D12Device* device)
{
	...
	// ブレンドステートの設定
	pipelineDesc.BlendState.RenderTarget[0].RenderTargetWriteMask = D3D12_COLOR_WRITE_ENABLE_ALL; 
	pipelineDesc.BlendState.RenderTarget[1].RenderTargetWriteMask = D3D12_COLOR_WRITE_ENABLE_ALL;
	...
	pipelineDesc.RTVFormats[1] = DXGI_FORMAT_R8G8B8A8_UNORM_SRGB; // 0～255指定のRGBA(SRGB版)
	pipelineDesc.SampleDesc.Count = 1; // 1ピクセルにつき1回サンプリング
...
PipelineSet Pipeline::CreatePostEffectPipeline(ID3D12Device* device)
{
	...
	result = D3DCompileFromFile(
		L"Resources/Shader/PostEffectPS.hlsl", // シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルード可能にする
		"main", "ps_5_0", // エントリーポイント名、シェーダーモデル指定
		D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, // デバッグ用設定
...
PipelineSet Pipeline::CreateMultiTexturePipeline(ID3D12Device* device)
{
	HRESULT result = S_FALSE;
	ComPtr<ID3DBlob> vsBlob; // 頂点シェーダオブジェクト
	ComPtr<ID3DBlob> psBlob;	// ピクセルシェーダオブジェクト
